...
func (m *expirationMap[V]) update(key, conflict uint64, oldExpTime, newExpTime time.Time) {
	if m == nil {
		return
	}
...
func (m *expirationMap[V]) cleanup(store store[V], policy policy[V], onEvict itemCallback[V]) {
	if m == nil {
		return
	}
